...
def get_args(config):
    args = argparse.ArgumentParser(
        '''Directions: Evaluating tool to play mario from the trained directroy.''')
    args.add_argument("--game", type=str, default="SuperMarioBros-Nes")
    args.add_argument("--state", type=str, default="Level1-1")
...
def run_test(data, opt):
    ...
    try:
        ...
        while True:
            ...
            score += reward
            if done:
                minute, second = divmod(400-info["time"], 60)
                print("model %s, time %dm%ds, score %.2f, evaluation %s."%(
                      data, minute, second, score, "success" if info["finish"] else "failure"))
                break
